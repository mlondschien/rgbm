...
def _mixed_batch(numerical_dtype=pa.float64(), dict_value_type=pa.string()):
    ...
    labels = features[:, 0] + (category_indices == 0).astype(np.float64) + 0.1 * rng.normal(size=num_rows)
    columns = {f"f{i}": pa.array(features[:, i], type=numerical_dtype) for i in range(features.shape[1])}

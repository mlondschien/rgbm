...
def test_lgbm_serialization_compatibility():
    ...
    y = num0 + 2.0 * (cat_strs == "a") + 1.0 * (cat_strs == "b") + rng.normal(scale=0.1, size=n)

    # rgbm's categorical bin codes equal Arrow's dictionary indices.
    X_lgbm = np.column_stack([num0, num1, cat_arr.indices.to_numpy().astype(np.float64)])

    ds = rgbm.Dataset(batch, pa.array(y, type=pa.float64()), max_bin=255, min_data_in_bin=3)
